...
## v1.26.0
- **⚠️ Richiede un aggiornamento manuale dello script Google e una nuova chiave**: questa versione aggiunge una chiave di sincronizzazione obbligatoria. Vai su Backup → guida in fondo: copia il nuovo codice, sostituisci il segnaposto con una chiave tua lunga e casuale, salva, rifai il deployment (Esegui il deployment → Gestisci i deployment → matita → Versione: Nuova versione → Deploy), poi incolla la stessa chiave nel nuovo campo "Chiave di sincronizzazione" in Backup — su ogni dispositivo. Finché non lo fai su tutti i dispositivi, quelli senza chiave smettono di sincronizzare (l'app continua comunque a funzionare in locale).
- **Sicurezza: chiusa la falla per cui chiunque conoscesse l'URL dell'Apps Script poteva leggere o scrivere sul foglio.** Ora ogni richiesta (invio e lettura) deve includere la chiave scelta: senza, il foglio la rifiuta.
- **Corrette possibili righe duplicate quando due dispositivi sincronizzano nello stesso istante**: lo script Google ora usa un blocco (lock) che fa aspettare il secondo tentativo finché il primo non ha finito di scrivere.
- **Google Sheets non trasforma più da solo note, nomi categoria o altri campi di testo** che assomigliano a una data o un numero (es. una nota "8/3"): tutte le colonne non numeriche vengono ora forzate a formato testo.
- **Corretto un bug di vecchia data nella protezione delle transazioni appena sincronizzate**: la v1.25.0 aveva introdotto un periodo di "grazia" per le transazioni appena create, basato sull'orario nascosto dentro il loro ID — ma quell'orario resta quello della *creazione*, quindi una transazione vecchia che viene *modificata* non ne beneficiava. Ogni transazione, categoria e risparmio porta ora un vero timestamp dell'ultima modifica, aggiornato a ogni salvataggio: la protezione vale sempre, sia per le creazioni sia per le modifiche.
